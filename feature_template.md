...
6. **정책 & 오류 규격 강화** : `추가 구현`
   - **비즈니스 정책 체계화**: PointPolicy 클래스로 최대/최소 금액, 단위, 잔고 정책 중앙 관리
   - **ErrorCode 확장**: 정책별 세분화된 에러 코드 추가 (EXCEED_MAX_BALANCE, INVALID_AMOUNT_UNIT 등)
   - **BusinessException 적용**: 서비스 레이어에서 IllegalArgumentException 대신 BusinessException 사용
   - **정책 검증 테스트**: PointPolicyValidationTest로 모든 정책 경계값 및 에러 케이스 검증 (200+ lines)
   - **HTTP 응답 일관성**: PointControllerHttpResponseTest로 HTTP 상태 코드와 에러 메시지 포맷 일관성 검증 (250+ lines)
...
- **정책과 예외 처리의 일관성**: 비즈니스 정책 강제화와 HTTP 응답 포맷의 일관성 유지를 위한 체계적 접근의 필요성
...
- **정책 중심 설계 패턴**: 비즈니스 정책을 상수 클래스로 중앙화하여 일관성 있는 검증 제공
- **MockMvc를 활용한 HTTP 응답 검증**: JSON Path를 이용한 구조화된 응답 검증과 상태 코드 일관성 테스트
...
- **정책 중심 예외 처리 체계 구축**:
  - 문제: 산발적인 예외 처리와 비일관적인 HTTP 응답 포맷
  - 해결: PointPolicy로 정책 중앙화, BusinessException으로 예외 표준화, MockMvc로 HTTP 응답 일관성 검증
>>>>>>> Stashed changes
